...
```
The variable selection and renaming are trivial, so we do not include them in the html file. 

```{r}
talkdata$Reward[1:10] # question: 5 cents or 10 cents?
```
One issue: the reward in the description section in the homework is 0.1, but the reward variable in the data is actually 0.05. Maybe the description is wrong.
...
```
We detect six suspicious observations in ages. *Eighteen (18)* and *27`* are irregular but valid responses, so we impute in their regular forms; other four cannot be identified, so we exclude them in the following analysis.
...
```
All six observations detected suspicious in gender are all missing. Although other values for these six observations are not missing, we still exclude them. Our justification is that these observations cannot provide information in the following analysis regarding gender. An alternative is that we do not exclude them, but actually it makes almost no difference because the relative number of samples are quite small.
...
```
It's plausible that the first five options in education can cover all education levels. Thus the last two options can be considered as not informative, enough for the justification of exclusion. 
...
```
We delete all observations with at least one missing in income, sirius and wharton. One important finding is that two people fill in "Yes" in wharton while "No" in sirius. This should not happen under our survey setting. It indicates that maybe some surveyors do the survey causally, which casts some doubt on the quality of the survey.
...
```
For the two integer variables age and worktime, we report their summary statistics table and histograms. Both variables are right-skewed. The valid sample size is 1723. We can see that most participants finish this survey with half a minute; also they are young, mostly aging from 20 to 35, possibly because they are more familiar with the Internet.
...
```
We illustrate the barplots of five categorical variables: gender, education, income, sirius and wharton. \textbf{Notes}: We map the education and income levels into integers for better exhibition. For education, 1 means \textit {less than 12 years; no high school diploma}, 2 means \textit{	
...
The key variables, sirius and wharton are of our primary interest. Over two thirds of the participants have ever listened to Sirius Radio, indicating that the distribution of the surveys is not inefficient -- what we care about is the answer to wharton conditional on a "Yes" in sirius. 

As for the covariates, it is revealed that the number of males is significantly larger than that of females among the surveyors. Their education levels are generally not high: few of them have a degree higher than bachelor. the income levels of the surveyors concentrate on the interval of \$15,000 - \$75,000 in 2014. It is worth noting that a considerable proportion of surveyors earn less than \$15,000 per year in 2014. The implication is, the sample is affected by self-selection bias, because only people registering as workers in MTurk are exposed to this survey. People with low income and low education level may have less opportunity cost for becoming a surveyor, so the sample may tilt towards them. Since income and education are plausibly correlated with the exposure to wharton business radio programs, the final estimate can be biased.
...
```
There is no missing value in the dataset. It is well cleaned beforehand.
...
11 years' data, 2006-2016, are included in the dataset. It is consistent to the data description.
...
## Comparison
We test the preditive power of three alternatives in addition: payroll to winning percentage (a replication of the one we've seen in class), payroll to increase in winning percentage next year, and payroll to increase in winning percentage current year. 
```{r}

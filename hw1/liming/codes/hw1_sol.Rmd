...
```
We detect six suspicious observations in ages. \textit{Eighteen (18)} and {27`} are irregular but valid responses, so we impute in their regualar forms; other four cannot be identified, so we exclude them in the following analysis.
...
```
For the two integer variables age and worktime, we report their summary statistics table and histograms. Both variables are right-skewed. The valid sample size is 1723. We can see that most participants are 

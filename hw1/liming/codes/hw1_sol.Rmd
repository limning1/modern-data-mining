...
We collect the demographic data for the US in 2014 from US Census Bureau.

### Age
```{r}
us_age = fread("G:/github/modern-data-mining/hw1/data/us_age.csv", encoding = "UTF-8")
us_age = data.table(melt(us_age))
us_age = us_age %>% 
  filter(variable != "Children 0-18")
us_age[,sum := sum(value),by = .(Location)]
us_age[,percent := value/sum]
setnames(us_age,"variable","age")

us_gender = fread("G:/github/modern-data-mining/hw1/data/us_gender.csv", encoding = "UTF-8")
us_gender = data.table(melt(us_gender))
setnames(us_gender,"variable","gender")

us_edu = fread("G:/github/modern-data-mining/hw1/data/us_edu.csv", encoding = "UTF-8")

us_income = read_excel("G:/github/modern-data-mining/hw1/data/us_income.xlsx")
us_income = us_income %>% 
  filter(!is.na(Total)) %>%
  melt() %>%
...
ggplot(us_age) + 
  geom_bar(aes(x=age,weight = percent))
```

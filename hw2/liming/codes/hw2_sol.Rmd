...
`Horsepower`, `displacement` and `weight` may need a transformation; the pairwise plot tells us that mpg may be linearly correlated to their reciprocals. However, when we consider log(mpg) instead of mpg, the non-linearity seems to alleviate. 

We have a closer look at the comparison between mpg and log(mpg) in terms of their relationship with the three variables.
...
The correlation between mpg and all three variables become more linear after transformation, so our model will consider log(mpg) instead of the original mpg.


```{r,eval=F}
# mpg.hist = ggplot(Auto,aes(x=mpg))+
#   geom_histogram()+
#   theme_bw()+
#   labs(title = "Histogram for MPG")
# cylinders.bar = ggplot(Auto,aes(x=cylinders))+
#   geom_bar()+
#   theme_bw()+
#   labs(title = "Barplot for Cylinders")
# displacemnet.hist = ggplot(Auto,aes(x=displacement))+
#   geom_histogram()+
#   theme_bw()+
#   labs(title = "Histogram for Displacement")
# horsepower.hist = ggplot(Auto,aes(x=horsepower))+
#   geom_histogram()+
#   theme_bw()+
#   labs(title = "Histogram for Horsepower")
# weight.hist = ggplot(Auto,aes(x=weight))+
#   geom_histogram()+
#   theme_bw()+
#   labs(title = "Histogram for Weight")
# acceleration.hist = ggplot(Auto,aes(x=acceleration))+
#   geom_histogram()+
#   theme_bw()+
#   labs(title = "Histogram for Acceleration")
# year.hist = ggplot(Auto,aes(x=year))+
#   geom_histogram()+
#   theme_bw()+
#   labs(title = "Histogram for Year")
# origin.bar = ggplot(Auto,aes(x=origin))+
#   geom_bar()+
#   theme_bw()+
#   labs(title = "Barplot for Origin")
# grid.arrange(mpg.hist,cylinders.bar,displacemnet.hist,horsepower.hist,weight.hist,acceleration.hist,year.hist,origin.bar,nrow = 3)
```
...
stargazer::stargazer(fit2, type="text", align=TRUE)
```
...
```{r}
ggplot(Auto1) + geom_boxplot(aes(x = cylinders, y = mpg))
```
...
### Describe the final model: use mpg
...
```{r}
Auto.new = data.table(Auto.new)

Auto.new[,":="(horsepower = 1/horsepower, displacement = 1/displacement, weight = 1/weight)]

setnames(Auto.new,c("horsepower","displacement","weight"),c("horsepower.rec","displacement.rec","weight.rec"))
```
...
fit.final2<-lm(log.mpg~cylinders+horsepower+weight+year*horsepower+origin,reg.current)
stargazer(fit.final2,type="text", align=TRUE)

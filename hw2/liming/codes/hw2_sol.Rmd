...
$$
\begin{aligned}
log.mpg = \alpha &+ \Sigma_i \beta_{1i}cylinder_i + \beta_{2}horsepower + \beta_3 weight + \Sigma_i \beta_{4} year \\&+ \Sigma_i \beta_{5i} year_i \times horsepower + \Sigma_i \beta_{6i} origin_i + \epsilon.
\end{aligned}
$$

The detailed specification and result of our final model: 
...
As to cylinders, let 3 cylinders be the benchmark, controlling all other explanatory variables. The mean mpg with 3 cylinders is 20.6. On average We expect, the mpg with 4 cylinders will increase by 2.93% compared to the mpg with 3 cylinders; the mpg with 5 cylinders will increase by 3.35% compared to the mpg with 3 cylinders; the mpg with 6 cylinders will increase by 2.02% compared to the mpg with 3 cylinders; the mpg with 8 cylinders will increase by 26.0% compared to the mpg with 3 cylinders. It may be the case that more cylinders can ensure a more complete burning of the fuels. 

As to weight, When vehicle weight increases by 1 lbs, holding others unchanged, we expect, on average the mpg will decrease by 0.02%. This is intuitive as heavier substances need a larger force to move against the friction. 

As to year, When year increases by 1 unit, given others the same, we expect, on average the mpg will increase by 4.71%. It may reflect that the advancement of technology, which makes all cars more efficient in fuel usage.

As to horsepower, When engine horsepower increases by 1 unit and all other explanatory variables are unchanged, we expect, on average the mpg will increase by 1.21%.

As to origin, let American be the benchmark, controlling others. The mean mpg with American origin is 20. On average We expect, the mpg with European origin will increase by 4.71% compared to the mpg with American origin; the mpg with Japanese origin will increase by 6.29% compared to the mpg with American origin. This possibly reflects the efficiency of cars from three regions: Japanese cars are the most efficient ones while American cars are the least.

As to displacement and acceleration, given existing explanatory variables, their coefficients are not significantly nonzero. They may have few direct effect on mpg.

There is also some interaction effect of year and horsepower. The negative coefficient indicates that, as time goes by, the positive correlation between mpg and horsepower is weaker. 
